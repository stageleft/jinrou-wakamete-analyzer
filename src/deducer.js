...
let job_list = ['村人', '占い', '霊能', '狩人', '共有', '猫又'];
let mob_list = ['村人', '人外', '人狼', '狂人', '妖狐', '子狐'];
let seer_result = ['', '○', '●'];
let medium_result = ['', '○', '●', '△', '■'];
let cast_list = [
  {
  ...
  }, // 11 villager without werefox
  {
    ...
    werefox: 1,
    minifox: 0,
  }, // 12 villager with increased werefox
  {
    seer: 1,
    medium: 1,
    bodyguard: 1,
    ...
    werefox: 1,
    minifox: 0,
  }, // 13 villager with increased werefox
  {
    seer: 1,
    medium: 1,
    bodyguard: 1,
...
export function updateInput(arg) {
  ...
  Object.keys(arg.log[base_date].players).forEach(function (k) {
    // 参加者別の情報
    let job = document.getElementById('job-' + k); // deduced Job
    let mrk = document.getElementById('mrk-' + k); // Monster Mark
    ...
    for (let i = 2; i <= datearray.length; i++) {
      let datestr = String(i) + '日目の朝となりました。';
      ret.each_player[k][datestr] = {};
      let target_obj = document.getElementById('stat-' + k + '-' + String(i) + '-target');
      ret.each_player[k][datestr].target = target_obj == null ? null : target_obj.value;
      let result_obj = document.getElementById('stat-' + k + '-' + String(i) + '-result');
...
export function updateInputField(arg) {
  ...
  //     'input' key of Web Storage API style
  let is_initialize = false; // if initialize Input Field
  let no_input_key = false; // if initialize 'input' key of Web Storage API style
  ...
  if (document.getElementById('deduce').textContent == '' || document.getElementById('deducer-title') == null) {
    ...
    let tds_title = document.getElementById('deducer-title').querySelectorAll('td');
  ...
  let player_list = Object.keys(arg.log[base_date].players);
  player_list.forEach(function (k) {
    ...
    for (let i = 2; i <= datearray.length; i++) {
      if (arg.log[datearray[i - 1]].players[k].stat == '（生存中）') {
        ...
        // if dead
        let target = document.getElementById('stat-' + k + '-' + String(i) + '-target');
        if (target != null) {
          target.remove();
        }

        let result = document.getElementById('stat-' + k + '-' + String(i) + '-result');
        if (result != null) {
          result.remove();
        }
    ...
    if (document.getElementById('villager-list-' + k).style.visibility != 'collapse') {
      ...
      let job = document.getElementById('job-' + k).value; // deduced Job
      let mrk = document.getElementById('mrk-' + k).value; // Monster Mark
      if (mrk == '村人') {
        if (job == '占い' || job == '霊能') {
          document.getElementById('villager-list-' + k).style.visibility = 'visible';
        } else if (job == '村人') {
  ...
  if (no_input_key == false) {
    ...
    let villager_cell_info = makeGrayVillagerList(arg);
    ...
    player_list.forEach(function (k) {
      ...
      for (let i = 2; i <= datearray.length; i++) {
        let datestring = datearray[i - 1];
        let alive_status = arg.log[datestring].players[k].stat;
        let target_label = document.getElementById('stat-' + k + '-' + String(i) + '-target-label');
        let target = document.getElementById('stat-' + k + '-' + String(i) + '-target');
        let result_label = document.getElementById('stat-' + k + '-' + String(i) + '-result-label');
        let result = document.getElementById('stat-' + k + '-' + String(i) + '-result');
        ...
        if (alive_status == '（生存中）') {
          if (
            is_initialize == true || // just after refreshInputField() called.
            ...
            if (job == '占い') {
              ...
              if (i >= 3) {
                ...
                target.style.visibility = 'visible';
                let voted_player = arg.log[datestring].list_voted[0];
                let o = document.createElement('option');
              ...
              // if (job == "村人" || job == "狩人" || job == "共有" || job == "猫又")
              // deducer: target (alive player list)
              target_label.innerText = '';
              target.setAttribute('disabled', 'disabled');
              target.style.visibility = 'collapse';
...
export function refreshInputField(arg) {
  ...
  //    </tr>
  //  </thead>
  let ret_head = document.createElement('thead');
  let tr_title = document.createElement('tr');
  tr_title.setAttribute('id', 'deducer-title');

  let td_day0title = document.createElement('td');
  ...
  for (let i = datearray.length; i >= 2; i--) {
    let td = document.createElement('td');
    td.setAttribute('id', 'deducer-title-' + String(i));
    td.setAttribute('colspan', '3');

    let a_day = document.createElement('a');
  ...
  player_list.forEach(function (k) {
    ...
    // deducer: deduced Job
    let td_co = document.createElement('td');
    td_co.innerText = 'ＣＯ';
    let deduced_job = document.createElement('select');
    ...
    let td_mm = document.createElement('td');
    td_mm.innerText = '推理';
    let monster_mark = document.createElement('select');
    ...
    for (let i = datearray.length; i >= 2; i--) {
      ...
      let dead_reason = document.createElement('p');
      ...
      let result_label = document.createElement('p');
